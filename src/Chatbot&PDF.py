...
def setup_vectorstoreforpdf(documents):
    ...
    vectorstore.save_local("faiss_gemini_store")
    print("done setting up vectorstore")
    return vectorstore

def load_existing_vectorstore():
    # if os.path.exists("faiss_groq_store/index.faiss"):
    #     embeddings = HuggingFaceEmbeddings(
    #         model_name="sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2"  # or whichever model you're using
    #     )
    #     return FAISS.load_local("faiss_groq_store", embeddings, allow_dangerous_deserialization=True)
    # return None
    if os.path.exists("faiss_gemini_store/index.faiss"):
        embeddings = GoogleGenerativeAIEmbeddings(
            model='models/text-embedding-004',
            task_type="retrieval_query"
        )
        return FAISS.load_local("faiss_gemini_store", embeddings, allow_dangerous_deserialization=True)
    return None
...
def parse_user_intent(text):
    ...
    if "cancel" in text:
        if any(word in text for word in ["status", "all", "show", "list", "fetch", "cancelled"]):
            return {"intent": None}
...
if user_input:
    ...
    # Conversational UPDATE flow
        ...
        # Step 2: Get new plan name
            ...
            selected_plan = next(
                (p for p in SubscriptionData.available_plans if p["name"].lower() == user_input.lower()), None)
            if selected_plan:
        ...
        if st.session_state.pending_action != "get":
            st.session_state.pending_action = "get"
            st.session_state.pending_payload = {}
    ...
    elif uploaded_files is None and st.session_state.pending_action == "None" :
        vectorstore = setup_vectorstore()
        print("intent none and uploaded file none")
        if vectorstore:
