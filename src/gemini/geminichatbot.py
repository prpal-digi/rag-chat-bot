...
load_dotenv()
print("Groq API Key:", os.getenv("google_api_key"))
...
def create_chain(vectorstore):
    ...
    # Use Gemini 1.5 Flash model via LangChain's wrapper
    llm = ChatGoogleGenerativeAI(
        model="gemini-2.0-flash",  # Most recent Flash model
        temperature=0,
        convert_system_message_to_human=True  # Ensures compatibility with LangChain message formats
    )
...
def parse_user_intent(text):
    text = text.lower()
    # Intent keywords
    if any(phrase in user_input.lower() for phrase in["cancel", "abort", "nevermind", "stop", "exit"]):
        return {"intent": "abort"}
...
if "vectorstore" not in st.session_state:
    ...
    st.session_state.vectorstore = setup_vectorstore(None)
...
if user_input:
    ...
    elif parsed["intent"] is None and uploaded_files is None:
        vectorstore = setup_vectorstore(None)
        if vectorstore:
